...
def denoise_sample(img, remove_fraction=0.35):
    """
    ...
    - remove_fraction: float, optional
        The fraction of high-frequency components to remove in the frequency domain.
        Default is 0.35
    ...
    for band in range(3):
        ...
        r, c = im_fft2.shape
        im_fft2[int(r*remove_fraction):int(r*(1-remove_fraction))] = 0
        im_fft2[:, int(c*remove_fraction):int(c*(1-remove_fraction))] = 0
        filtered_img[:, :, band] = fftpack.ifft2(im_fft2).real
    return filtered_img

def denoise_dataset(dataset, remove_fraction=0.35):
    """
    ...
    for i in range(dataset.shape[0]):
        filtered_dataset[i] = denoise_sample(dataset[i], remove_fraction)
